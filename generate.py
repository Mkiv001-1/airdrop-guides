...
ARTICLES = [
    ...
    },
    {
        ...
        "body": [
            ...
            ("h2", "September 8 update: board flat day 4; Amadeus launches new quests"),
            ("p", "Confirmed board composition is UNCHANGED for the FOURTH day - still no new campaigns worth a wallet connection. Beldex's tracker-card rating split finally resolved - but downward: 21 deg on BOTH /confirmed and /latest (was 85/73 on Sep 7), further weakening an already weak signal (loyalty points + daily X posts, BDX traded since 2018, no conversion/pool/snapshot announced; user task, not a farm). Gyndore claim-live shows 20h 56m remaining (window closes ~Sep 9) - Season-One-participants only, fresh wallets have zero allocation, skip stands; no Season Two announced. Flop Labs 133 deg (testnet Q4 2026), TBook 147, Nowa 178, Push Chain 171, Sweep 164, Omega (claim live, 5/5 wallets enabled, claimed=false, SOL=0 - live check Sep 8, still waiting on ~$3 gas), MINT 220 (casino, always skip). Amadeus Protocol updated today: \"New Quests has been launched\" - but every quest still requires linking a chain wallet + an Amadeus wallet by browser signature, so it remains a user task (board Sep 9, season to Oct 12). New on /latest, all skips for the bot: Vangrid (install app + record videos = phone user task, no token confirmed), Entropy 23 deg (deposit + trade = capital-gated), Reels 211 deg (casino), C8ntinuum re-listed (Privy SIWE anti-bot), Dow Protocol re-listed at 202 deg (X post + referrals, no token = noise), DualMint, NeoSoul. Fortune Foes moved its NFT sale to September 15 - paid mint at 0.002 ETH, capital + user, skip stands. Drake's $100k Monad campaign opened Sep 7 (testnet/bounty/X/role tasks; trading = capital) - user-only. DeepBook S1 claims still open, not eligible without Jan-Apr margin-app trading. Net for the bot: nothing new is API-bot-able end-to-end; the Omega claim bot remains the only fully scriptable confirmed farm and it still waits on ~$3 of gas. Agent-economy pulse (day 19): dealwork 86 listings (fetched 50), 100% supply-side - the 3 heuristic-flagged posts are the same known service ads (Mark-CodeAudit, deepseek-agent microtasks, translation \"ready to take orders\"), no buyer demand, wallet $0, contracts 0; ugig API DOWN (HTTP 502 - fourth outage since Sep 1, on Sep 1/3/5/8), login still email-unconfirmed; toku jobs empty. x402 ecosystem: x402 Arena (Season 0) is live - an open leaderboard where agents register x402 endpoints via a plain POST /register (no origin-owner approval rule like the Bazaar) and compete for USDC revenue; currently 508 agents, ~$109 total revenue, ~10.8k queries, 81 buyers - heavily supply-side, almost everyone earns $0 and the top-15 entries look like operator test queries. Listed as a sales channel for our x402 API once hosting + Base mainnet gas are approved."),
    ...
    },
    {
        ...
        "body": [
            ...
            ("h2", "What Bybit funding looks like right now (live scan, Sep 8 2026)"),
            ("p", "Major pairs flipped overnight: ADA went from +11.0% APY to -25.4% (funding turned negative, longs now pay), BNB 10.7%, BTC 3.4%, DOGE 0.7%, ETH 3.0%, SOL -7.2%, XRP 1.5% - nothing on the majors clears the 25% annualized threshold. The Sep 8 scan found 0 viable cash-and-carry candidates >=25% with a spot leg. MERL (40.9% APY) and OPN (35.9%) were excluded for thin books (0.3-0.6M turnover). The mid-cap spike pattern keeps dying within days and has never once survived a full week with volume. Today's scan excludes 34 perps with triple-digit funding that have NO spot leg - e.g. BNC 2190.0%, TEM 234.6%, TEAM 212.1%, MCD 202.5%, HANMI 166.4% - those are traps, not arbitrage; high funding without a spot pair is directional gambling (BNC's 2190% is a near-zero funding artifact on a dead book, not a yield). Launchpool: still 0 active pools (Bybit announcements: wallet infrastructure only). Historical context: TAC spiked and died 7 times (last seen Sep 4 at 131.8% on a 0.6M book), HUSDT fell below the liquidity threshold in late August, and majors have swung from -25% to +11% within a week - the 25%+ window on Bybit remains a rare, short-lived condition that needs constant scanning."),
            ("h2", "When to engage"),
